...
func TestManagedFormDecodeValidateAndRoundTrip(t *testing.T) {
	...
	restored, err := DecodeManagedForm("form.yaml", strings.NewReader(canonical), configuration)
	if err != nil {
		t.Fatal(err)
	}
...
func TestManagedFormRejectsUnsupportedFormatAndDepth(t *testing.T) {
	...
	if err := ValidateManagedForm("form.yaml", form, configuration); !errors.Is(err, ErrUnsupportedFormat) {
...
func BenchmarkDecodeManagedForm(b *testing.B) {
	...
	b.ResetTimer()
	for range b.N {
		if _, err := DecodeManagedForm("form.yaml", bytes.NewReader(source.Bytes()), configuration); err != nil {
			b.Fatal(err)
		}
	}
}
