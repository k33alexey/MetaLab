(() => {
  ...
  window.createBSLEditor = textarea => {
    ...
    let file=null,sourceText='',analysis=null,active=false,timer=0,generation=0,controller=null,selectedPair=null,selectedPairKey='',folded=null,completionTimer=0,completionGeneration=0,completionController=null,completionResult=null,completionSelection=0,breakpoints=new Set(),executionLine=0;

    function open(nextFile){
      generation++;cancelCompletion();if(controller)controller.abort();clearTimeout(timer);file=nextFile;sourceText=nextFile.content;analysis=null;selectedPair=null;selectedPairKey='';folded=null;executionLine=0;active=nextFile.language==='bsl';textarea.readOnly=false;host.classList.toggle('bsl-active',active);host.classList.remove('editor-folded');panel.hidden=!active;properties.hidden=active;actions.forEach(action=>action.hidden=!active);inspectorTitle.textContent=active?'BSL':'Свойства';summary.textContent='';summary.className='muted';code.textContent=active?sourceText+'\n':'';renderGutter();renderInspector();if(active)schedule(0)
    }

    function close(){generation++;cancelCompletion();if(controller)controller.abort();clearTimeout(timer);file=null;sourceText='';analysis=null;active=false;folded=null;executionLine=0;selectedPair=null;selectedPairKey='';textarea.readOnly=false;host.classList.remove('bsl-active','editor-folded');panel.hidden=true;properties.hidden=false;actions.forEach(action=>action.hidden=true);inspectorTitle.textContent='Свойства';summary.textContent='';summary.className='muted';code.textContent='';gutter.replaceChildren()}
    ...
    async function navigate(mode){
      if(!active||!file)throw new Error('Откройте модуль BSL.');hideCompletion();const response=await fetch('/api/bsl/navigate',{method:'POST',headers:{'Content-Type':'application/json','X-ML-CSRF':'1'},body:JSON.stringify({path:file.path,content:sourceText,position:cursorPosition(textarea.selectionStart),mode})});if(!response.ok)throw new Error((await response.text()).trim());return response.json()
    }
    async function rename(newName){
      if(!active||!file)throw new Error('Откройте модуль BSL.');hideCompletion();const response=await fetch('/api/bsl/rename',{method:'POST',headers:{'Content-Type':'application/json','X-ML-CSRF':'1'},body:JSON.stringify({path:file.path,position:cursorPosition(textarea.selectionStart),newName,expectedRevision:file.revision})});if(!response.ok)throw new Error((await response.text()).trim());return response.json()
    }

    async function help(){
      if(!active||!file)throw new Error('Откройте модуль BSL.');hideCompletion();const response=await fetch('/api/bsl/help/resolve',{method:'POST',headers:{'Content-Type':'application/json','X-ML-CSRF':'1'},body:JSON.stringify({path:file.path,content:sourceText,position:cursorPosition(textarea.selectionStart)})});if(!response.ok)throw new Error((await response.text()).trim());return response.json()
    }
    function setBreakpoints(lines){breakpoints=new Set(lines||[]);renderGutter()}
    function setExecutionLine(line){executionLine=line||0;renderGutter()}
    function routineAtCursor(){if(!active||!analysis)return'';const line=cursorPosition(textarea.selectionStart).line,candidates=(analysis.symbols||[]).filter(item=>(item.kind==='function'||item.kind==='procedure')&&item.range.start.line<=line);return candidates.length?candidates[candidates.length-1].name:''}
    ...
    function renderGutter(){
      ...
      const appendLine=line=>{const row=document.createElement('div');row.className='gutter-line';if(typeof line==='number'&&breakpoints.has(line))row.classList.add('has-breakpoint');if(line===executionLine)row.classList.add('execution-line');const fold=typeof line==='number'?foldByLine.get(line):null;if(fold){const button=document.createElement('button');button.type='button';button.textContent=folded===fold?'+':'−';button.title=folded===fold?'Развернуть блок':`Свернуть: ${fold.label}`;button.addEventListener('click',event=>{event.stopPropagation();toggleFold(fold)});row.append(button)}else row.append(document.createElement('span'));const number=document.createElement(typeof line==='number'?'button':'span');number.className='gutter-number';number.textContent=typeof line==='number'?String(line):'⋯';if(typeof line==='number'){number.type='button';number.title=breakpoints.has(line)?'Удалить точку останова':'Добавить точку останова';number.addEventListener('click',()=>{const enabled=!breakpoints.has(line);if(enabled)breakpoints.add(line);else breakpoints.delete(line);renderGutter();window.dispatchEvent(new CustomEvent('ml-bsl-breakpoint',{detail:{path:file?.path,line,enabled}}))})}row.append(number);fragment.append(row)};
