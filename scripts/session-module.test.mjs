...
test('the Studio page opens the session module from its own tree node',()=>{
  for(const match of html.matchAll(/<script>([\s\S]*?)<\/script>/g))new vm.Script(match[1]);
  assert.match(html,/node\.id==='session-module'/);
  assert.match(html,/fetch\('\/api\/session-module',\{method:'POST'/);
  // Открытие создаёт файл, поэтому идёт как мутация — с тем же заголовком,
  // что и остальные изменяющие запросы Studio.
  assert.match(html,/'\/api\/session-module',\{method:'POST',headers:\{'X-ML-CSRF':'1'\}\}/);
  // После создания дерево перечитывается: состояние узла меняется.
  const opener=html.slice(html.indexOf('async function openSessionModule'),html.indexOf('// Узел группы «Роли»'));
